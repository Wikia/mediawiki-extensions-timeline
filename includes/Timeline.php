...
class Timeline implements ParserFirstCallInitHook {
	...
	public static function onTagHook( ?string $timelinesrc, array $args, Parser $parser ): string {
		...
		// Fandom change - start
		// Allow to override default upload dir via $wgTimelineUploadDir
		// @author t-tomalak - PLATFORM-4961
		$pathPrefix = $backend->getContainerStoragePath( ltrim( $wgTimelineUploadDir, '/' ) ) . "/$hash";
		// Fandom change - end
	...
	private static function renderTimeline( string $timelinesrc, array $options ): void {
		...
		if ( !is_file( $svgFile ) ) {
			// lazydev: fail closed; add compiler diagnostics if missing SVG files become common.
			LoggerFactory::getInstance( 'timeline' )->warning(
				'EasyTimeline SVG output is missing before upload verification',
				[ 'svgFile' => $svgFile ]
			);
			throw new TimelineException( 'timeline-error-storage' );
		}

		$svgSize = filesize( $svgFile );
		if ( $svgSize === false ) {
			LoggerFactory::getInstance( 'timeline' )->warning(
				'EasyTimeline SVG output size could not be read before upload verification',
				[ 'svgFile' => $svgFile ]
			);
			throw new TimelineException( 'timeline-error-storage' );
		}
		...
		// lazydev: UCP exposes these upload classes globally; switch namespaces when core does.
